    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        for j, a in enumerate(args):
            if isinstance(a, str):
                with contextlib.suppress(ValueError):
                    try:
                        args[j] = locals()[a] if a in locals() else globals()[a] if a in globals() else ast.literal_eval(a)
                    except:
                        args[j] = a
        ...
        if m in (
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3):
                ...
                n = 1
        elif m in (FGM, OmniKernel, CSPOmniKernel, SOEP):
            c2 = ch[f]
            args = [c2, *args]
        elif m in (AIFI,):
            c2 = ch[f]
            args = [ch[f], *args]
        elif m in (nn.MaxPool2d, nn.Upsample):
            c2 = ch[f]
