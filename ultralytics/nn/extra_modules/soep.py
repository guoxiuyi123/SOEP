...
class CSPOmniKernel(nn.Module):
    ...
    def forward(self, x):
        ok_branch, identity = torch.split(
            self.cv1(x), [int(x.size(1) * self.e), int(x.size(1) * (1 - self.e))], dim=1
        )
        return self.cv2(torch.cat((self.m(ok_branch), identity), 1))


class SOEP(nn.Module):
    def __init__(self, dim, csp=False, e=0.25):
        super().__init__()
        self.block = CSPOmniKernel(dim, e=e) if csp else OmniKernel(dim)
